...
func linuxATSLeaseDeadline(config []byte, command linuxCommand) (int64, bool) {
	if !linuxATSLeasePattern.MatchString(command.LeaseID) {
		return 0, false
	}
	...
	for _, row := range profile.Outbounds {
		...
		case "pokrov-ats":
			if read(row, "type") != "pokrov-ats-lease" ||
				...
				read(row, "active_flows_until") != command.ActiveFlowsUntil {
				return 0, false
			}
			gate++
		case "pokrov-ats-upstream":
			if read(row, "type") != "vless" {
				return 0, false
			}
			upstream++
		}
	}
	if gate != 1 || upstream != 1 {
		return 0, false
	}
