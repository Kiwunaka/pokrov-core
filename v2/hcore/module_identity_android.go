...
func readAndroidModuleSHA256() string {
	...
	if unix.Fstat(int(file.Fd()), &before) != nil || before.Mode&unix.S_IFMT != unix.S_IFREG ||
		...
		uint64(unix.Minor(uint64(before.Dev))) != mapping.minor {
		return ""
	}
	...
	if inAPK {
		...
		if match == nil || match.Method != zip.Store || match.UncompressedSize64 == 0 ||
			match.UncompressedSize64 > 1<<30 || match.CompressedSize64 != match.UncompressedSize64 {
			return ""
		}
	...
	if size <= 0 || size > 1<<30 || start < 0 || start > before.Size-size {
		return ""
	}
	...
	for _, program := range image.Progs {
		if program.Type != elf.PT_LOAD || program.Flags&elf.PF_X == 0 ||
			virtual < program.Vaddr || virtual-program.Vaddr >= program.Filesz {
			continue
		}
		delta := virtual - program.Vaddr
		if program.Off >= uint64(size) || delta >= uint64(size)-program.Off {
			return ""
		}
		fileOffset := uint64(start) + program.Off + delta
		if fileOffset < mapping.offset || fileOffset-mapping.offset != address-mapping.start {
			return ""
		}
		matched = true
		break
