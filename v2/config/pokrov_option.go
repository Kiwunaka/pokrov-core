...
func DefaultPokrovOptions() *PokrovOptions {
	return &PokrovOptions{
		EnableNTP: true,
...
func GetOverridablePokrovOptions(overrides map[string][]string) *PokrovOptions {
	overridePokrov := PokrovOptions{}

	// Convert flat overrides to nested structure
	nestedOverrides := convertFlatToNested(overrides)

	// Use reflection to iterate over the fields of PokrovOptions
	v := reflect.ValueOf(&overridePokrov).Elem()
	t := reflect.TypeOf(overridePokrov)

	// Recursively set the fields that are marked as overridable
	setOverridableFields(v, t, nestedOverrides)

	return &overridePokrov
}
