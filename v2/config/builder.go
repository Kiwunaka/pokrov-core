...
func BuildConfig(ctx context.Context, hopts *PokrovOptions, inputOpt *ReadOptions) (*option.Options, error) {
...
func setOutbounds(options *option.Options, input *option.Options, opt *PokrovOptions, staticIPs *map[string][]string) error {
	...
	for _, out := range input.Outbounds {
		...
		default:
			...
			out = *patchPokrovWarpFromConfig(&out, *opt)
	...
	if opt.Warp.EnableWarp {
		...
		// out, err := GenerateWarpSingbox(wg, opt.Warp.CleanIP, opt.Warp.CleanPort, &option.WireGuardPokrov{
		...
		// })
		out, err := GenerateWarpSingboxNew("p1", &pokrov.NoiseOptions{})
		if err != nil {
...
func setExperimental(options *option.Options, hopt *PokrovOptions) {
...
func setLog(options *option.Options, opt *PokrovOptions) {
...
func setInbound(options *option.Options, hopt *PokrovOptions) {
...
func setRoutingOptions(options *option.Options, hopt *PokrovOptions) error {
	...
	// 	// 			PackageName: []string{"app.pokrov.com"},
	...
	// 	// 		Type: C.RuleTypeDefault,
	// 	// 		DefaultOptions: option.DefaultRule{
	// 	// 			ProcessName: []string{"Pokrov", "Pokrov.exe", "PokrovCli", "PokrovCli.exe"},
	// 	// 			Outbound:    OutboundBypassTag,
	// 	// 		},
	// 	// 	},
	// 	// )
...
func patchPokrovWarpFromConfig(out *option.Outbound, opt PokrovOptions) *option.Outbound {
	if out.Type == C.TypePsiphon {
		return out
	}
