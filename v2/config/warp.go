...
func generateWarp(license string, host string, port uint16, noise *pokrov.NoiseOptions) (*T.Endpoint, error) {
...
func GenerateWarpSingbox(wgConfig WarpWireguardConfig, host string, port uint16, noise *pokrov.NoiseOptions) (*T.Endpoint, error) {
	if host == "" {
		host = "auto4"
	}
...
func GenerateWarpSingboxNew(uniqueIdentifier string, noise *pokrov.NoiseOptions) (*T.Endpoint, error) {
...
func patchWarp(base *option.Endpoint, configOpt *PokrovOptions, final bool, staticIpsDns map[string][]string) error {
	...
	if final && base.Type == C.TypeWireGuard {
		if opts, ok := base.Options.(*option.WireGuardEndpointOptions); ok {
			...
			if opts.Detour != "" {
				...
				opts.Noise = pokrov.NoiseOptions{}
