...
func convertRange(s string) pokrov.Range {
	r := pokrov.Range{}
	r.UnmarshalJSON([]byte(strconv.Quote(s)))
	return r
}
func getWireGuardNoise(d map[string]string, addDefault bool) pokrov.NoiseOptions {
	...
	if fake_packet_count.To == 0 && fake_packet_delay.To == 0 && fake_packet_size.To == 0 && fake_packet_mode == "" {
		...
		return pokrov.NoiseOptions{}
	}
	return pokrov.NoiseOptions{
		FakePacket: pokrov.FakePacketOptions{
			Enabled: true,
...
func defaultWireguardNoiseOptions() pokrov.NoiseOptions {
	return pokrov.NoiseOptions{
		FakePacket: pokrov.FakePacketOptions{
			Enabled: true,
